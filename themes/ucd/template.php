...
function ucd_theme(&$existing, $type, $theme, $path) {
  $registry = array();
  $template_dir = drupal_get_path('theme', 'ucd') . '/templates';
  ...
  $registry['ucd_main_menu_links'] = array(
    'render element' => 'element',
    'function' => 'theme_ucd_main_menu_links',
    'variables' => array(),
  );
  $registry['ucd_secondary_menu_links'] = array(
    'render element' => 'element',
    'function' => 'theme_ucd_secondary_menu_links',
    'variables' => array(),
  );
  $registry['ucd_footer_menu_links'] = array(
    'render element' => 'element',
    'function' => 'theme_ucd_footer_menu_links',
    'variables' => array(),
  );
  return $registry;
}
 ...
 */
function ucd_preprocess_html(&$vars) {
  global $base_url;
  // Add web fonts from fonts.com
  $element = array(
    '#tag' => 'link', // The #tag is the html tag - <link />
    '#attributes' => array( // Set up an array of attributes inside the tag
      'href' => '//writingcenter.ucdenver.edu/demo.css',
      'rel' => 'stylesheet',
      'type' => 'text/css',
    ),
  );
  ...
  foreach ($touch_icons as $touch_icon) {
    ...
    $href = $base_url . '/' . drupal_get_path('theme', 'ucd') . '/apple-icon-' . $sizes . '.png';
    $element = array(
      '#tag' => 'link',
      '#attributes' => array(
  ...
  $vars['path_to_ucd'] = drupal_get_path('theme', 'ucd');
  ...
  // Add focus js
  drupal_add_js(drupal_get_path('theme','ucd') .'/js/track-focus.js', array('scope' => 'footer'));
 ...
 */
function ucd_preprocess_page(&$vars) {
  global $base_url;
  ...
  // add print logo
  $vars['print_logo'] = '<img src="' . $base_url . '/' . drupal_get_path('theme','ucd') . '/images/print-logo.png" alt="University of Colorado Boulder" />';
  // hide title on homepage
  if($vars['is_front'] == TRUE) {
    $vars['title_hidden'] = TRUE;
  }
  // add home icon to footer menu if needed
  ucd_home_icon($vars, 'footer_menu');
  ...
  $classes = ucd_size_column_classes();
 ...
 */
function ucd_preprocess_node(&$vars) {
 ...
 */
function ucd_preprocess_region(&$vars) {
  global $base_url;
  ...
  switch ($vars['region']) {
    case 'branding':
      ...
      $vars['svg_logo'] = $base_url . '/' . drupal_get_path('theme', 'ucd') . '/images/cu-logo.svg';
 ...
 */
function ucd_process_block(&$vars) {
 ...
 */
function ucd_preprocess_block(&$vars) {
 ...
 */
function ucd_menu_link(array $vars) {
 ...
 */
function theme_ucd_main_menu_links($vars) {
 ...
 */
function theme_ucd_secondary_menu_links($vars) {
 ...
 */
function theme_ucd_footer_menu_links($vars) {
  $classes = join(' ',$vars['attributes']['class']);
 ...
 */
function ucd_menu_local_tasks(&$vars) {
  $output = '';
 ...
 */
function ucd_qt_quicktabs_tabset($vars) {
  $variables = array(
    'attributes' => array(
 ...
 */
function ucd_size_column_classes() {
  $classes = array();
  $classes['xs'] = array(
    1 => 'col-xs-12',
    2 => 'col-xs-12',
 ...
 */
function ucd_home_icon(&$vars, $menu) {
  if (isset($vars[$menu])) {
...
function ucd_affiliation($site_type = NULL, $value = NULL) {
  $affiliations = array(
    'sport_club' => array(
 ...
 */
function ucd_breadcrumb($vars = NULL) {
